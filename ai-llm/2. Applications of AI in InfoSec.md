...
```
The above code shows where the `DATA_BASE_PATH` holding our data resides. The `TARGET_BASE_PATH` is where the split data will reside.

When `splitfolders.ratio` is called, it makes an 80-20 split (based on `TRAINING_RATIO`) in the newly created `TARGET_BASE_PATH`. There is no validation data in the above (owing to 0)
...
## Confusion Matrix
The confusion matrix provides a detailed breakdown of the model's predictions, showing each class' number of **true positives**, **true negatives**, **false positives**, and **false negatives**.

Can be trivially produced using `sklearn`
```python
...
## ResNet50 CNN
This model is 50 layers deep, where it got its name, and consists of roughly 23 million parameters. This model is strong in image classification tasks.
